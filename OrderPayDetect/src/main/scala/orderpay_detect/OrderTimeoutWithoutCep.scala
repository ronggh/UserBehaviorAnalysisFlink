...
//        val timeoutWarningStream = orderEventStream.process( new OrderTimeoutWarning() )
    val orderResultStream = orderEventStream.process(new OrderPayMatch(orderTimeoutOutputTag))
...
//
class OrderPayMatch(orderTimeoutOutputTag:OutputTag[OrderResult]) extends KeyedProcessFunction[Long, OrderEvent, OrderResult] {
