...
// 定义输入订单事件的样例类，eventType：create/pay
...
object OrderTimeout {
  def main(args: Array[String]): Unit = {
    ...
    // 2. 定义一个匹配模式，检测15分钟内订单是否支付成功
    val orderPayPattern = Pattern.begin[OrderEvent]("created").where(_.eventType == "create")
...
class OrderTimeoutSelect() extends PatternTimeoutFunction[OrderEvent, OrderResult] {
  override def timeout(map: util.Map[String, util.List[OrderEvent]], l: Long): OrderResult = {
    val timeoutOrderId = map.get("created").iterator().next().orderId
...
class OrderPaySelect() extends PatternSelectFunction[OrderEvent, OrderResult] {
  override def select(map: util.Map[String, util.List[OrderEvent]]): OrderResult = {
    val payedOrderId = map.get("payed").iterator().next().orderId
